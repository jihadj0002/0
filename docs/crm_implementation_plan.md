...
- [x] 6.8 New training module `module-10-crm-operations.md` — "how to use the CRM app" for sales staff: roles/visibility matrix, dashboard widgets + 5-min morning routine, leads (quick add, filters, score buckets 70/40, drawer actions), pipeline kanban rules (won/lost conversion, lost reasons), follow-ups (overdue-first, every touch sets next), call logging (8 outcomes table), meetings/demos status discipline, calendar/tasks, customers (package/renewal/notes, churn risk), companies, scripts/FAQ/Learn resources, team/reports (honest data), notifications + global search, 6 golden rules, daily rhythm, self-test. Wired into TRAINING-INDEX (Day 10), playbook curriculum (Day 10 check "logs a real lead live"), module-07 checklist; re-seeded both DBs (21 articles / 46 FAQs)

### Phase 7 — Bug fixes & UX round (2026-08-05)
- [x] 7.1 Leads visibility: staff already scoped to assigned+unassigned (`lead_queryset_for`) — added the **Assigned filter (Mine+Unassigned / Assigned to me / Unassigned)** for staff in `leads.html` (was owner/manager-only)
- [x] 7.2 Assign-to-me: drawer (`_lead_popup.html`) now shows **"⚡ Assign to Me"** when a lead is unassigned and viewable; `ajax_quick_update` accepts `field=assigned_to` (`"me"` → self, managers may pass any staff pk); uses existing `update_lead` assignment logging + new-assignee notification; actions wired globally in `crm.js` so they work from both Leads and Pipeline drawers
- [x] 7.3 Drawer stage-change 404 fixed: `leads.html` posted to `/crm/ajax/leads/<id>/quick-update` but the route is `ajax/leads/<int:pk>/update` — moved handler to global `crm.js` posting to the correct `/update` URL (this also gave Pipeline's drawer a working stage form, which it never had)
- [x] 7.4 Script deactivate 500 fixed: JS posted `/crm/scripts/<pk>/toggle` without trailing slash → APPEND_SLASH + POST in DEBUG raises RuntimeError; now posts `/toggle/` (route unchanged)
- [x] 7.5 Demo scheduling save error fixed: `{% csrf_token %}` added to `base.html` (sets CSRF cookie on every CRM page — `Crm.api` POSTs on demos/scripts/calls/followups relied on it but most pages never rendered a token); `demos` POST hardened — invalid datetime returns 400 JSON (was raw 500 `ValueError`), lead pk parsed as int, platform validated against choices, XHR requests get a JSON response instead of a redirect
- [x] 7.6 Tasks: staff can no longer assign tasks to other staff (view forces `assigned_to=user`); managers/owners keep the assignee picker. New `ajax_task_update` endpoint (`crm/urls.py`) — status/title/priority/deadline/assigned_to, permission: managers any, staff own/unassigned/created-by only. `tasks.html` rewritten: per-row status select, edit (✎) modal, delete kept; **completed tasks hidden by default** (status filter: Open hides done, "Done"/"Cancelled"/"All" reveal them); deadline parsed via `parse_datetime` (was a 500 vector)
- [x] 7.7 Script viewer formatting: `data-content` escapejs strings now decoded with `JSON.parse` before render (literal `\n`/`\uXXXX` garbage gone); `.script-viewer-body` keeps `white-space: pre-wrap` so line breaks render
- [x] 7.8 Tier system (docs-only, per decision): added "How the Tier System Works" to `PIPELINE.md` — Tier 1 = 10K–50K followers call-first, Tier 2 = 5–10K DM, Tier 3 = rest; tiers apply only to 🔵 New leads and expire on first contact; no CRM field added
- [x] 7.9 Regression tests in `crm/tests.py` (see below)
- [x] 7.10 Drawer "not working" (mobile + desktop) — root cause: **static files are served from the R2 CDN in BOTH dev and prod** (`.env` has R2 config → `STATIC_URL = https://<CLOUDFLARE_R2_PUBLIC_DOMAIN>/static/`), and the bucket's `crm/js/crm.js` was the OLD copy. The old crm.js had no delegated `.stage-form` submit handler (it lived in leads.html and posted to the removed `/quick-update`), so clicking "Update" in the drawer did a **native GET form submit → full page reload with `?stage=N` → drawer closed**, looking broken on every device; the drawer's Assign-to-Me button also did nothing. Fixed by uploading the new `crm/js/crm.js` to the R2 bucket (`s3.put_object`, `ContentType=application/javascript`, `ACL=public-read`) and verified in headless Chrome (1280×800 desktop + 390×844 mobile touch): drawer opens, stage update stays in place, assign-to-me claims the lead, zero console errors. Also: `refreshLeadRow` now updates the row's Assigned To cell (`data-assigned-cell`) after assign/unassign. **Operational note: after changing anything under `crm/static/`, re-upload the changed files to the R2 bucket — Django `{% static %}` points at the CDN in dev too.**

### Phase 8 — Admin oversight, script editing & assignment lockdown (2026-08-06)
- [x] 8.1 Leads page: owners/managers can now filter by **each staff member** (`assigned=<staff_pk>` in `views.leads`, dropdown lists every active staff) — "who is on what stage" at a glance; staff still get only Mine+Unassigned / Me / Unassigned. Sort links and pagination now preserve **all** filters (`qs_params` context var instead of just `q`)
- [x] 8.2 Sales script popup fixed properly: root cause was `|escapejs` — it escapes `"`/`\` but **not newlines**, and the HTML attribute parser collapses literal newlines to spaces → popup showed the whole script as one run-on paragraph. New `json_attr` filter (`crm_extras.py`) = `escape(json.dumps(value))` → newlines survive as `\n` in the attribute, `JSON.parse` recovers them, viewer renders with `white-space: pre-wrap`; content is also HTML-escaped before innerHTML (was an XSS vector). Copy button now falls back to `document.execCommand` on insecure contexts (`Crm.copyText` resolves a boolean)
- [x] 8.3 Assignment lockdown for staff: `lead_new` forces `assigned_to=user` (was: staff could assign new leads to any staff via tampered POST), `lead_detail` action=update only accepts self/unassigned for staff (was: any staff pk), and the detail page's Assigned To select shows only "— Unassigned — / (Me)" for staff (full staff list for managers). Also fixed: the detail-page update guard `lead.assigned_to_id == user.id` excluded unassigned leads, so staff could NOT claim an unassigned lead from the detail form (drawer assign-to-me already worked) — now `in (user.id, None)`
- [x] 8.4 Convert to Customer: button was owner/manager-only and "missing" for staff. Now shown for any `can_edit` (lead owner + managers) on `lead_detail.html` ("🏆 Mark Won & Convert") and in the drawer (`_lead_popup.html` → `Crm.convertModal`). Conversion also marks the lead **Won** (stage saved directly + activity log, bypassing `update_lead`'s auto-convert which would drop the package/monthly value — that was a latent bug in the old convert flow too)
- [x] 8.5 Script editing: owner/manager get a **✏️ Edit** button per script card → prefilled modal → AJAX save to new `POST /crm/scripts/<pk>/edit/` (`script_edit` view: title required, category validated, JSON responses); "+ New Script" modal is now AJAX too (`scripts` POST returns JSON when `X-Requested-With: XMLHttpRequest`, which `Crm.api` now always sends; native form POST still redirects). 26 new tests: `LeadAssigneeFilterTests`, `AssignmentGuardTests`, `ConvertFlowTests`, `ScriptEditTests` — suite at **83 tests, all passing**; verified end-to-end in headless Chrome (owner filter → per-staff rows, unassigned filter, script popup 4-line rendering, edit modal prefilled + save, AJAX new script, staff detail page select restriction + convert modal, drawer convert button, zero console errors). `crm/js/crm.js` re-uploaded to R2 CDN (12825 bytes)
