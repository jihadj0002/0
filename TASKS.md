...
| [x] | P1 | @api | **BUG — duplicate Integration rows / wrong one returned.** `_connect_page` upserted on `(user, platform, integration_id)` so it created a 2nd row beside the tokenless manual one → `.filter(platform).first()` picked the wrong/disconnected row. **Fixed:** `_connect_page` now upserts on `(user, platform)` (one row per platform), added `Integration.get_active(user, platform)` (connected-first, else most-recent), and routed `sender.py`/`pipeline.py`/per-user webhook/`back/views.py` token lookups through it. Verified on `test1`: connect → still 1 row, resolver prefers the connected one. |
| [x] | P1 | @api | **BUG — page webhook subscription didn't take (had to set it in the dashboard).** Root cause was a malformed `META_APP_ID` in `.env` (`= "id"` → value `' "id"'` with stray space+quotes) → invalid `client_id` and a broken `{app_id}|{app_secret}` app token, so subscription calls failed. **Fixed** the `.env` line. Also made `subscribe_page` return `{ok,status,body}` + log/surface the real Graph error, and added `manage.py setup_meta_webhook [--dry-run] [--callback-url]` to configure the app-level `page`/`instagram` webhook subscription via the App Subscriptions API (automates the dashboard step; callback must be publicly reachable). |
