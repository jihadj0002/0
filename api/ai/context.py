...
def build_system_prompt(user, conversation):
    ...
    parts.append(
        ...
        "1. Translate Bengali to English first ('moshari'→'mosquito net', "
        "'jama'→'dress'/'frock'); try original + translation. Brand names unchanged.\n"
        "2. Vary keywords across searches: shorter terms (drop extra words), and "
        "synonyms ('dress'→'frock','party dress','skirt'; 'shirt'→'t-shirt','top'). "
        "Don't just repeat the same query.\n"
        "3. CHECK NAMES against the request. A 'dress' is NOT 'dress shoes'; a "
        "'bathtub' is NOT a 'mosquito net'. Present ONLY genuine matches.\n"
        "4. If no genuine match after 2–3 tries, say it's out of stock — do NOT list "
    ...
    parts.append(
        ...
        "Search using it — try the Name first, then brand+type, then key words "
