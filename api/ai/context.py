...
def build_system_prompt(user, conversation):
    ...
    parts.append(
        ...
        "Call search_products immediately when the customer asks about a product.\n"
        "You MUST search at least 2 times (preferably 3) with different keywords "
        "before saying something is unavailable.\n"
        ...
        "Example: 'jama' → translates to 'dress' or 'frock'. "
        "Try BOTH the original term and the English translation "
        "in separate searches if you're unsure. Brand names stay unchanged.\n"
        "2. After search_products returns: check product NAMES against what the "
        "customer asked. If the customer wants a 'dress' but results are "
        "'dress shoes' or 'caps' — those are NOT matches — search AGAIN.\n"
        "3. If a long query returns nothing relevant (e.g. 'birthday dress 1 year "
        "girl' has no results), try SHORTER queries: remove words one at a time. "
        "Example: try 'birthday dress', then 'dress', then try COMPLETELY "
        "different keywords like 'party dress', 'frock', 'skirt'.\n"
        "4. Try synonyms and related terms. 'Dress' → 'frock', 'party dress', "
        "'skirt', 'overall'. 'Shirt' → 't-shirt', 'panjabi', 'top'.\n"
        "5. NEVER present products whose names don't match the customer's request. "
        "If all searches return irrelevant items, say it's out of stock.\n"
        "6. Only ask the customer for clarification after 3+ failed searches.\n"
    )
    if external_catalog:
        parts.append(
